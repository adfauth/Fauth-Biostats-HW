...
Observing with the `str()` shows me that I have a df with 60 rows, one character variable, and two numeric variables.

Also, looking at the Treatment variable, it looks categorical. Lets's make it a factor.
...
sum(is.na(df$Dried_mass_g)) ## counting all the T values
```
...
Our response variable type is: Growth_rate_mm (continuous)

Therefore our statistical method is: ANOVA (3 levels in the treament)
...
Looks like there will be a discernible effect of treatment and that level 1 and level 2 will be different from control. Not sure if level 1 and level 2 will differ from one another.
...
We have evidence of a discernible effect of treatment on 
